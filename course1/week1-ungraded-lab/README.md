...
Important: Only the `docker` method of deployment is verified for this lab. The updated `Dockerfile` and `docker-compose.yml` ran successfully on Apple M1. 
...
The repo is updated to use docker-compose and Dockerfile:
* Dockerfile uses the latest jupyter-notebook as base
* docker-compose.yml sets up the docker configuration

To start the server use `docker-compose up` and copy/paste the notebook url to your browser
To update the docker image, use `docker-compose up --build`
...
### DEPRECATED: 1. Pulling the image from Docker hub
...
### DEPRECATED 2. Running a container out of the image:
Note: these arguments are now built into the Dockerfile and docker-compose
